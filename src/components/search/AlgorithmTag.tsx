...
import { tagList } from '../../data/tagDummyData';

type AlgorithmTagProps = {
  tags: string[];
  setTags: React.Dispatch<React.SetStateAction<string[]>>;
  onTag: () => void;
};

export default function AlgorithmTag({
  tags,
  setTags,
  onTag,
}: AlgorithmTagProps) {
  ...
  const filteredTags = tagList.filter(
    (v) => v.includes(query) && !tags.includes(v),
  );
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (e.key === 'ArrowDown') {
      ...
      if (highlightIndex >= 0 && highlightIndex < filteredTags.length) {
        handleAddTag(filteredTags[highlightIndex]);
      }
    }
  };
    ...
    <>
      ...
      {open && (
        ...
        >
          ...
          >
            <div
              className={`relative mb-2 flex rounded-sm border bg-white ${hasText ? 'border-[#2d95ad]' : 'border-gray1'}`}
            >
              <input
                type="text"
